...
Page({
  ...
  getTable: function(uid, pwd, showCookieClass, cookie, vcode) {
    var that = this;
    wx.request({
      url: app.globalData.apiURL + '/teacher/courseTable.php',
      method: "POST",
      header: {
      ...
      success: function(res) {
        ...
        if (res.data.status == 401) {
          wx.navigateTo({
            url: '/pages/error/queryerror?ErrorTips=' + "工号密码不对，请重新登录",
          })
        }
  ...
  goClassPlace: function(ep) {
    ...
    var result = placeArr.indexOf(ep.target.dataset.place.slice(0, -11));
    // console.log(result);
    wx.navigateTo({
  ...
  refreshData: function() {
    wx.redirectTo({
      url: '/pages/index/vcode?to=teacherKb&update=1',
    })
  }
});